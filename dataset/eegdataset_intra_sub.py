...
def load_data(eeg_path, img_path, nSub, seed, batch_size, batch_size_test):
    dataset = EEGDataset_train(eeg_path, img_path, nSub)
    train_dataset, val_dataset = random_split(dataset, [len(dataset) - 740, 740], generator=torch.Generator().manual_seed(seed))
...
class EEGDataset_train(TensorDataset):
    ...
    def load_eeg(self):
        data_file = self.eeg_path + '/sub-' + format(self.nSub, '02') + '/preprocessed_eeg_training.npy'
    ...
    def __getitem__(self, idx):
        ...
        label_edge = self.img_edge[idx]
        label_depth = self.img_depth[idx]
        label_scene = self.img_scene[idx]
